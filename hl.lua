-- Lua script to highlight strings in an Nvim buffer using perl instead of vim regex tool (/)
...
local perl_regex = 'local'                                           -- Capture the perl regex query
...
local match_table = {} -- Lua table that will be used later to keep record of the current cursor position and next/prev cursor position (for the functionality
                       -- to scroll forward/backward through the highlighted regex-matches using 'n' and 'N')
...
local match_row_max, match_col_max = match_table[#match_table][1], match_table[#match_table][2] -- Coordinates of the last match in the nvim buffer (for wrapping around the scroll when using 'n')
...
function goto_nxt_perl_match(match_table)                            -- Function to make the cursor go to the next regex match (useful for scrolling between matches)
   ...
   if (
      ...
      for i, v in ipairs(match_table) do
         local match_row, match_col = v[1], v[2]                     -- Loop through each match's coordinates and stop at a point on the first match satisfying the if condition below
                                                                     -- to get the next and nearest match relative to the cursor position
         if (
